...
#one-col-entry(
  content: [
    #two-col(left-column-width: design-highlights-summary-left-margin, right-column-width: 1fr, left-content: [], right-content: [#v(design-highlights-top-margin);#align(left, [Production-grade multi-platform homelab infrastructure using Kubernetes \(Talos Linux\), Proxmox, TrueNAS, and hybrid cloud services.])], column-gutter: 0cm)

#v(-design-text-leading)  #v(design-highlights-top-margin);#highlights([#strong[Multi-platform architecture:] Orchestrated 2 Kubernetes clusters with Sidero Omni \(S3 backups\), Proxmox hypervisor, and TrueNAS storage serving NFS\/iSCSI to 11+ applications across 280+ declarative configuration files.],[#strong[Hybrid cloud networking:] Deployed Pangolin server \(AWS EC2\) with Newt agent in Kubernetes for WireGuard-based, identity-aware reverse proxying of internet-exposed services; implemented Tailscale mesh VPN for private service access.],[#strong[Multi-cloud security:] Integrated Azure Key Vault for secret management \(Pangolin auth, database credentials\), Azure Blob Storage for database backups, and AWS S3 for cluster snapshots with SOPS encryption for GitOps compliance.],[#strong[HA database infrastructure:] Implemented CloudnativePG with 6+ PostgreSQL clusters \(3-instance HA\) featuring automated Barman backups and 14-day retention.],[#strong[Advanced networking:] Configured Cilium CNI with eBPF L2 load balancing, Kubernetes Gateway API, Cloudflare Tunnels, and automated DNS management via ExternalDNS.],[#strong[Automated CI\/CD:] Built FluxCD GitOps pipeline with Mend Renovate Bot for continuous dependency updates and Prometheus\/Grafana observability stack.],)
  ],
)
